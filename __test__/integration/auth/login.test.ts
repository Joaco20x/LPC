...
describe('procesarLogin', () => {
  ...
  it('retorna tokens y datos del usuario para credenciales válidas', async () => {
    ...
    usuarioRepo.buscarPorCorreo.mockResolvedValue({
      ...
      contrasenaHash: '$2a$12$hashed', verificado: true,
    });
    bcryptMock.compare.mockResolvedValue(true as never);

    const resultado = await procesarLogin('juan@test.com', 'password123', usuarioRepo, sesionRepo);
  ...
  it('lanza error si el usuario no existe', async () => {
    ...
    await expect(procesarLogin('no@existe.com', 'pass123', usuarioRepo, sesionRepo))
  ...
  it('lanza error si el usuario no tiene contrasenaHash (OAuth)', async () => {
    const { usuarioRepo, sesionRepo } = crearMocks();
    usuarioRepo.buscarPorCorreo.mockResolvedValue({
      id: 'user-2', nombre: 'Maria', correo: 'maria@test.com',
      contrasenaHash: null, verificado: true,
    });

    await expect(procesarLogin('maria@test.com', 'pass', usuarioRepo, sesionRepo))
      .rejects.toThrow('Credenciales incorrectas');
  });
  ...
  it('lanza error si la contraseña es incorrecta', async () => {
    ...
    await expect(procesarLogin('juan@test.com', 'wrongpass', usuarioRepo, sesionRepo))
      .rejects.toThrow('Credenciales incorrectas');
  });
});
