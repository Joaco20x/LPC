import { crearGrupoViaje, obtenerGruposDelUsuario, obtenerDetalleGrupo } from '@/grupos/services/grupos.service';
...
describe('crearGrupoViaje', () => {
  it('crea grupo y miembros en una transacción', async () => {
    ...
    await crearGrupoViaje(
      {
        nombre: 'Viaje', pais: 'Chile', fechaInicio: '2026-07-01', fechaFin: '2026-07-10',
        idCreador: 'user-1', correosIntegrantes: ['invitado@test.com'],
      },
      usuarioRepo, grupoRepo, miembroRepo, db,
    );
  ...
  it('lanza error si hay correos no registrados', async () => {
    ...
    await expect(crearGrupoViaje(
      {
        nombre: 'Viaje', pais: 'Chile', fechaInicio: '2026-07-01', fechaFin: '2026-07-10',
        idCreador: 'user-1', correosIntegrantes: ['no-existe@test.com'],
      },
      usuarioRepo, grupoRepo, miembroRepo, db,
    )).rejects.toThrow('Uno o más correos ingresados no corresponden a usuarios registrados');
  });
});
    ...
    miembroRepo.buscarPorUsuario.mockResolvedValue([
      {
        rol: 'admin',
        grupo: {
          id: 'g1', nombre: 'Viaje Chile', destino: 'Chile',
          fechaInicio: new Date('2026-07-01'), fechaFin: new Date('2026-07-10'),
          monedaBase: 'CLP',
          _count: { miembros: 3 },
        },
      } as any,
...
describe('obtenerDetalleGrupo', () => {
  ...
  it('lanza error si el grupo no existe', async () => {
    ...
    await expect(obtenerDetalleGrupo('no-existe', grupoRepo))
