...
describe("validarDescripcion", () => {
  ...
  it("retorna error si es menor a 3 caracteres", () => {
    expect(validarDescripcion("ab")).toBe(
      "La descripción debe tener al menos 3 caracteres",
    );
  });

  it("retorna error si supera 255 caracteres", () => {
    expect(validarDescripcion("a".repeat(256))).toBe(
...
describe("validarGrupo", () => {
  ...
  it("retorna error si está vacío", () => {
    expect(validarGrupo("")).toBe(
      "El gasto debe estar asociado a un grupo válido",
    );
  });

  it("retorna error si es null/undefined", () => {
    expect(validarGrupo(null)).toBe(
      "El gasto debe estar asociado a un grupo válido",
    );
  });
});
...
describe("validarGasto", () => {
  ...
  it("retorna error de descripción en validarGasto", () => {
    const errores = validarGasto({
      idGrupo: "g1",
      descripcion: "ab",
    });
  ...
  it("retorna error de categoría en validarGasto", () => {
    const errores = validarGasto({
      idGrupo: "g1",
      categoria: "Invalida",
    });
    expect(errores.some((e) => e.campo === "categoria")).toBe(true);
  });
