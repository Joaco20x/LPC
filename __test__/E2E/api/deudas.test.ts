...
jest.mock('@/shared/di/crearDependencias', () => {
  ...
  return {
    crearDependencias: jest.fn(() => ({
      ...
      usuarioRepo: { buscarPorCorreo: jest.fn(), buscarPorId: jest.fn(), buscarPorEmails: jest.fn(), buscarPorOauth: jest.fn(), crear: jest.fn(), actualizarContrasena: jest.fn() },
      ...
      miembroGrupoRepo: { buscarPorGrupo: jest.fn(), crearMuchas: jest.fn(), buscarPorUsuario: jest.fn(), buscarMiembrosDeGrupos: jest.fn() },
...
describe('GET /api/deudas', () => {
  it('retorna 200 con deudas pendientes', async () => {
    ...
    deps.deudaRepo.obtenerPendientes.mockResolvedValue([
      {
        id: 'd1', idDeudor: 'user-test', idAcreedor: 'user-2', monto: 100,
  ...
  it('retorna 401 sin token', async () => {
    const req = crearMockNextRequest({ url: 'http://localhost:3000/api/deudas' });
    const respuesta = await controladorDeudas(req);
