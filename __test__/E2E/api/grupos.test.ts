import { controladorCrearGrupo, controladorObtenerGrupos, controladorObtenerDetalleGrupo } from '@/grupos/controllers/grupos.controller';
...
describe('POST /api/grupos', () => {
  it('retorna 201 para grupo válido', async () => {
    ...
    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/grupos',
      token: tokens.accessToken,
      cuerpo: {
        nombre: 'Viaje Chile', pais: 'Chile', fechaInicio: '2026-07-01',
        fechaFin: '2026-07-10', correosIntegrantes: ['invitado@test.com'],
      },
    });
...
describe('GET /api/grupos', () => {
  it('retorna 200 con grupos del usuario', async () => {
    ...
    deps.miembroGrupoRepo.buscarPorUsuario.mockResolvedValue([
      {
        rol: 'admin',
        grupo: { id: 'g1', nombre: 'Viaje', destino: 'Chile', fechaInicio: new Date(), fechaFin: new Date(), monedaBase: 'CLP', _count: { miembros: 2 } },
      } as any,
    ]);
...
describe('GET /api/grupos/[id]', () => {
  it('retorna 200 con detalle del grupo', async () => {
    const { crearDependencias } = require('@/shared/di/crearDependencias');
    const deps = crearDependencias();
    deps.grupoRepo.obtenerDetalle.mockResolvedValue({ id: 'g1', nombre: 'Viaje' });
  ...
  it('retorna 404 si el grupo no existe', async () => {
    ...
    const respuesta = await controladorObtenerDetalleGrupo(req, { id: 'no-existe' });
