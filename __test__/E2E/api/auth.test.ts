...
jest.mock('@/shared/di/crearDependencias', () => {
  ...
  return {
    crearDependencias: jest.fn(() => ({
      ...
      gastoRepo: { crear: jest.fn(), obtenerTodos: jest.fn(), obtenerPorId: jest.fn() },
...
const tokens = generarTokens({ idUsuario: 'user-integration', correo: 'user@test.com' });
...
describe('POST /api/auth/login', () => {
  it('retorna 200 para credenciales válidas', async () => {
    ...
    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/auth/login',
...
describe('POST /api/auth/registro', () => {
  it('retorna 201 para registro exitoso', async () => {
    ...
    deps.usuarioRepo.crear.mockResolvedValue({
      id: 'new-user', nombre: 'Nuevo', correo: 'nuevo@test.com',
      contrasenaHash: 'hash', verificado: false, creadoEn: new Date(),
    });

    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/auth/registro',
      cuerpo: { nombre: 'Nuevo', correo: 'nuevo@test.com', contrasena: 'password123', confirmarContrasena: 'password123' },
    });
  ...
  it('retorna 400 para datos inválidos', async () => {
    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/auth/registro',
      cuerpo: { nombre: '', correo: '', contrasena: '', confirmarContrasena: '' },
    });
...
describe('POST /api/auth/refresh', () => {
  it('retorna 200 con refresh token válido', async () => {
    const { crearDependencias } = require('@/shared/di/crearDependencias');
    const deps = crearDependencias();
    deps.sesionRepo.buscarPorTokenHash.mockResolvedValue({ id: 'session-1', idUsuario: 'user-1' });

    const { refreshToken } = tokens;
    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/auth/refresh',
...
describe('POST /api/auth/logout', () => {
  it('retorna 200 y cierra sesión', async () => {
    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/auth/logout',
...
describe('POST /api/auth/recuperar-contrasena', () => {
  it('retorna 200 para correo válido', async () => {
    ...
    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/auth/recuperar-contrasena',
...
describe('POST /api/auth/nueva-contrasena', () => {
  it('retorna 400 si falta token', async () => {
    const req = crearMockNextRequest({
      metodo: 'POST', url: 'http://localhost:3000/api/auth/nueva-contrasena',
...
describe('GET /api/auth/usuarios/buscar', () => {
  ...
  it('retorna 200 con token válido', async () => {
    ...
    const deps = crearDependencias();
    deps.usuarioRepo.buscarPorCorreo.mockResolvedValue({
      id: 'user-1', nombre: 'Encontrado', correo: 'encontrado@test.com',
    });
