...
try {
  ...
  for (const [filePath, metrics] of Object.entries(coverageData)) {
    ...
    const promedioArchivo = ((lines.pct + statements.pct + functions.pct + branches.pct) / 4).toFixed(2);
    
    reporte += `${path.relative(__dirname, filePath)}: ${promedioArchivo}%\n`;

    if (lines.pct < UMBRAL || statements.pct < UMBRAL || functions.pct < UMBRAL || branches.pct < UMBRAL) {
  ...
  const promedioGlobal = ((total.lines.pct + total.statements.pct + total.functions.pct + total.branches.pct) / 4).toFixed(2);
