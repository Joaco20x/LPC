...
export function validarGrupo(
  ...
  if (!idGrupo || idGrupo.trim() === "")
    return "El gasto debe estar asociado a un grupo válido";
  return null;
}
...
export function validarGasto(datos: DatosGasto): ErrorCampoGasto[] {
  ...
  if (errorDescripcion)
    errores.push({ campo: "descripcion", mensaje: errorDescripcion });

  const errorCategoria = validarCategoria(datos.categoria);
  if (errorCategoria)
    errores.push({ campo: "categoria", mensaje: errorCategoria });
  ...
  if (datos.divisiones && datos.divisiones.length > 0) {
    datos.divisiones.forEach((div, i) => {
      if (!div.idUsuario) {
        errores.push({
          campo: `divisiones[${i}].idUsuario`,
          mensaje: "El usuario de la división es requerido",
        });
      }
      if (div.montoAsignado < 0) {
        errores.push({
          campo: `divisiones[${i}].montoAsignado`,
          mensaje: "El monto asignado no puede ser negativo",
        });
      }
    });
    ...
    if (montoTotal > 0) {
      ...
      const divisionesExactas = datos.divisiones.filter(
        (d) => d.tipoDivision === "exacto",
      );
      if (divisionesExactas.length > 0) {
        const sumaExacta = divisionesExactas.reduce(
          (acc, d) => acc + d.montoAsignado,
          0,
        );
      ...
      const divisionesPorcentaje = datos.divisiones.filter(
        (d) => d.tipoDivision === "porcentaje",
      );
      if (divisionesPorcentaje.length > 0) {
        const sumaPct = divisionesPorcentaje.reduce(
          (acc, d) => acc + d.montoAsignado,
          0,
        );
