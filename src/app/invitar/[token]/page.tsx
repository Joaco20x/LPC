...
import { obtenerDatosUsuario, obtenerAccessToken } from '@/shared/servicios/almacenamientoTokens';
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Marca */}
        <p style={{ fontSize: '0.8rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: '#2d4a3e', marginBottom: '2rem' }}>
          Eatryp — Invitación
        </p>

        {cargando ? (
          <p style={{ color: '#6b6b67', textAlign: 'center' }}>Cargando invitación…</p>
        ) : error ? (
          <div>
            <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: '1.75rem', marginBottom: '1rem', color: '#1a1a18' }}>
              Invitación inválida
            </h1>
            <p style={{ color: '#c0392b', marginBottom: '2rem' }}>{error}</p>
            <Link href="/dashboard" style={{ color: '#2d4a3e', fontWeight: 500 }}>Ir al dashboard →</Link>
          </div>
        ) : exito ? (
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>🎉</div>
            <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: '1.75rem', color: '#2d4a3e', marginBottom: '0.5rem' }}>
              ¡Bienvenido al grupo!
            </h1>
          ...
          <>
            ...
            <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: '2rem', color: '#1a1a18', marginBottom: '0.25rem' }}>
              {info.nombreGrupo}
            </h1>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                ...
                <strong style={{ color: colorEstado[info.estado] }}>{etiquetaEstado[info.estado]}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                <span style={{ color: '#a0a09c' }}>Expira</span>
                <span style={{ color: '#1a1a18' }}>{formatearFecha(info.expiraEn)}</span>
              </div>
              {info.correoInvitado && (
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                  <span style={{ color: '#a0a09c' }}>Para</span>
                  <span style={{ color: '#1a1a18' }}>{info.correoInvitado}</span>
                </div>
              )}
            </div>
            ...
            {info.estado === 'pendiente' ? (
              estaLogueado ? (
                <div>
                  <p style={{ fontSize: '0.9375rem', color: '#6b6b67', marginBottom: '1.5rem' }}>
                    Hola, <strong>{usuarioActual?.nombre}</strong>. Haz clic para unirte al grupo.
                  </p>
                  <button
                ...
                <div>
                  <p style={{ fontSize: '0.9375rem', color: '#6b6b67', marginBottom: '1.5rem' }}>
                    Para unirte a este grupo necesitas iniciar sesión o crear una cuenta.
                  </p>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <Link href="/dashboard" style={{ color: '#2d4a3e', fontWeight: 500 }}>
                  Ir al dashboard →
                </Link>
              </div>
            )}
