...
export default function PaginaVotaciones() {
  ...
  const cargar = useCallback(async () => {
    setCargando(true);
    try {
      const res = await peticionAutenticada(`/api/votaciones?idGrupo=${idGrupo}`);
  ...
  const votar = async (idVotacion: string, decision: 'aprobar' | 'rechazar') => {
    setVotando(idVotacion);
    setMensaje(null);
    try {
      const res = await peticionAutenticada(`/api/votaciones/${idVotacion}/votar`, {
        method: 'POST',
  ...
  const crearVotacion = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.exito) {
        setMensaje({ tipo: 'ok', texto: 'Votación iniciada. Se notificó a todos los integrantes.' });
    ...
    <div className="dashboard-cuerpo votaciones-raiz">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p className="dashboard-encabezado__descripcion">Abstenciones y denuncias por deudas</p>
        </div>
        <button onClick={() => setMostrarForm(!mostrarForm)} className="votac-btn-crear">
          + Nueva votación
        </button>
      </div>
        ...
        <div className="votac-form">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div>
              ...
              />
              <p style={{ fontSize: '0.75rem', color: 'var(--color-texto-suave)', marginTop: '0.25rem' }}>
                Puedes obtener el ID desde la sección de Deudas del grupo.
              </p>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={crearVotacion} disabled={creando || !idDeuda.trim()} className="votac-btn-crear" style={{ flex: 1 }}>
                {creando ? 'Iniciando…' : 'Iniciar votación'}
              </button>
              <button onClick={() => setMostrarForm(false)} className="votac-btn-cancelar">
                Cancelar
              </button>
            </div>
          </div>
      ...
      {/* Lista de votaciones */}
      {cargando ? (
        <p style={{ color: 'var(--color-texto-suave)' }}>Cargando votaciones…</p>
      ) : error ? (
        ...
        <div className="votac-vacio">
          ...
          <p style={{ fontSize: '0.875rem', marginTop: '0.25rem' }}>Inicia una para proponer una abstención o denunciar una deuda.</p>
        </div>
      ) : (
            ...
            return (
              <div key={v.id} className="votac-card">
                ...
                <div className="votac-card__cabecera">
                  <div style={{ flex: 1 }}>
                    <span className="votac-badge-tipo">{etiquetaTipo[v.tipo]}</span>
                    <p style={{ fontSize: '0.75rem', color: 'var(--color-texto-suave)', marginTop: '0.25rem' }}>
                      Iniciada {new Date(v.creadoEn).toLocaleDateString('es-CL')}
                    </p>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span style={{
                      background: v.estado === 'activa' ? '#edf4f1' : '#f0f0f0',
                    ...
                    {v.resultado && (
                      <p style={{ fontSize: '0.8rem', fontWeight: 600, color: etiquetaResultado[v.resultado].color, marginTop: '0.25rem' }}>
                ...
                <div style={{ margin: '1rem 0' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8125rem', marginBottom: '0.4rem' }}>
                    <span style={{ color: '#2d4a3e', fontWeight: 500 }}>✔ Aprobar ({v.aprobaciones})</span>
                    <span style={{ color: '#6b6b67' }}>{v.pendientes} pendientes</span>
                    <span style={{ color: '#c0392b', fontWeight: 500 }}>✖ Rechazar ({v.rechazos})</span>
                  </div>
                  <div style={{ height: '10px', background: '#f0f0ee', borderRadius: '999px', overflow: 'hidden', display: 'flex' }}>
                    <div style={{ width: `${pctAprobar}%`, background: '#2d4a3e', transition: 'width 0.4s' }} />
                    <div style={{ width: `${pctRechazar}%`, background: '#c0392b', transition: 'width 0.4s' }} />
                  </div>
                  <p style={{ fontSize: '0.75rem', color: 'var(--color-texto-suave)', marginTop: '0.4rem' }}>
                    {v.totalMiembros} integrante{v.totalMiembros !== 1 ? 's' : ''} en el grupo
                  </p>
                </div>
                ...
                {v.votos.length > 0 && (
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                    {v.votos.map((vt) => (
                      <div key={vt.idUsuario} title={`${vt.nombreUsuario}: ${vt.decision}`} style={{
                        display: 'flex', alignItems: 'center', gap: '0.375rem',
                        background: vt.decision === 'aprobar' ? '#edf4f1' : '#fef0f0',
                        border: `1px solid ${vt.decision === 'aprobar' ? '#b8d4c8' : '#f5c6c6'}`,
                        borderRadius: '999px', padding: '0.2rem 0.6rem', fontSize: '0.75rem',
                        color: vt.decision === 'aprobar' ? '#2d4a3e' : '#c0392b',
                      }}>
                        <div style={{
                          width: '20px', height: '20px', borderRadius: '50%',
                          background: vt.decision === 'aprobar' ? '#2d4a3e' : '#c0392b',
                ...
                {v.estado === 'activa' && yaVote && (
                  <p style={{ fontSize: '0.875rem', color: 'var(--color-texto-suave)', fontStyle: 'italic' }}>
                    Ya votaste: <strong>{miVoto?.decision === 'aprobar' ? '✔ Aprobar' : '✖ Rechazar'}</strong>. Esperando al resto.
                  </p>
                )}
              </div>
            );
