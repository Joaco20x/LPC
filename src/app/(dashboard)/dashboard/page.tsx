    ...
    <div className="dashboard-cuerpo">
      <header className="dashboard-encabezado">
        ...
        <p className="dashboard-encabezado__descripcion">
          Desde aquí puedes gestionar tus viajes actuales o configurar nuevas aventuras con tu grupo.
        </p>
      </header>

      {/* ── Acceso rápido a deudas ── */}
      <section style={{ marginBottom: '1.5rem' }}>
        <Link href="/deudas" className="boton-solido" style={{ fontSize: '0.875rem', padding: '0.6rem 1.25rem' }}>
      ...
      <section style={{ marginBottom: '3rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <p className="dashboard-seccion__titulo" style={{ margin: 0 }}>Mis grupos de viaje</p>
          <Link href="/grupos/crear" className="boton-solido" style={{ fontSize: '0.875rem', padding: '0.6rem 1.25rem' }}>
        ...
        {cargandoGrupos ? (
          ...
          <div className="dashboard-vacio">
            ...
            <p style={{ color: 'var(--color-texto-secundario)' }}>{errorGrupos}</p>
          </div>
        ) : grupos.length === 0 ? (
          <div className="dashboard-vacio">
            <div className="dashboard-vacio__icono">
            ...
            <p className="dashboard-vacio__descripcion">
              Crea tu primer grupo de viaje y empieza a registrar gastos compartidos.
            </p>
          ...
          <div className="dashboard-grid">
            {grupos.map((grupo) => (
              <div key={grupo.id} className="dashboard-tarjeta" style={{ cursor: 'default' }}>
                {/* Etiqueta de rol */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <span className="dashboard-tarjeta__etiqueta">
                    {grupo.rol === 'admin' ? 'Admin' : 'Miembro'}
                  </span>
                  <span className="dashboard-tarjeta__etiqueta" style={{ background: '#edf4f1', color: 'var(--color-acento)' }}>
                    {grupo.totalMiembros} {grupo.totalMiembros === 1 ? 'persona' : 'personas'}
                  </span>
                </div>

                {/* Info del grupo */}
                <div className="dashboard-tarjeta__contenido">
                  <Link href={`/grupos/${grupo.id}`} style={{ textDecoration: 'none' }}>
                    <p className="dashboard-tarjeta__nombre" style={{ color: 'var(--color-acento)', cursor: 'pointer' }}>
                      {grupo.nombre}
                    </p>
                  </Link>
                  ...
                  <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)', marginTop: '0.5rem', fontWeight: 300 }}>
                    {formatearFecha(grupo.fechaInicio)} → {formatearFecha(grupo.fechaFin)}
                  </p>
                </div>
