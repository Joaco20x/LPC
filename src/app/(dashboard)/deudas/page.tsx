...
function TarjetaDeuda({ deuda, tipo }: { deuda: DeudaItem; tipo: 'debo_a' | 'me_deben' }) {
    ...
    <div className="dashboard-tarjeta">
        ...
        <span style={{ fontSize: '1.125rem', fontWeight: 600, color: colorMonto }}>
        ...
        <p className="dashboard-tarjeta__descripcion">{deuda.contraparte.correo}</p>
        <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)', marginTop: '0.5rem', fontWeight: 300 }}>
            Grupo: {deuda.grupo.nombre}
        </p>
        <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)', fontWeight: 300 }}>
...
function ContenidoDeudas() {
    ...
    const totalDeboA   = deudas?.debo_a.reduce((s, d) => s + d.monto, 0) ?? 0;
    ...
    <div className="dashboard-cuerpo">
        ...
        {grupoId && (
            <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)', marginTop: '0.5rem' }}>
        ...
        {cargando && (
        <div style={{ padding: '2rem 0', color: 'var(--color-texto-suave)', textAlign: 'center' }}>
            Cargando deudas...
        </div>
        )}
      ...
      {/* ── Contenido principal ── */}
        ...
        <>
          {/* Resumen numérico */}
            ...
            <div className="dashboard-tarjeta" style={{ borderLeft: '3px solid #c0392b' }}>
                <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)', marginBottom: '0.25rem' }}>
                Total que debo
                </p>
                <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)' }}>
                {deudas.debo_a.length} deuda{deudas.debo_a.length !== 1 ? 's' : ''}
                </p>
            </div>

            <div className="dashboard-tarjeta" style={{ borderLeft: '3px solid var(--color-acento)' }}>
                <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)', marginBottom: '0.25rem' }}>
                Total que me deben
                </p>
                <p style={{ fontSize: '1.5rem', fontWeight: 600, color: 'var(--color-acento)' }}>
                {formatearMonto(totalMeDeben)}
                </p>
                <p style={{ fontSize: '0.8125rem', color: 'var(--color-texto-suave)' }}>
                {deudas.me_deben.length} deuda{deudas.me_deben.length !== 1 ? 's' : ''}
                </p>
            </div>
            </div>
          ...
          {/* Sección: Debo a */}
            ...
            <p className="dashboard-seccion__titulo" style={{ marginBottom: '1.5rem' }}>Debo a</p>
            {deudas.debo_a.length === 0
                ? <EstadoVacio titulo="¡Sin deudas!" descripcion="No tienes deudas pendientes con nadie." />
                : (
          ...
          {/* Sección: Me deben */}
            <section>
            <p className="dashboard-seccion__titulo" style={{ marginBottom: '1.5rem' }}>Me deben</p>
            {deudas.me_deben.length === 0
                ? <EstadoVacio titulo="Nadie te debe" descripcion="No hay deudas pendientes hacia ti." />
                : (
                <div className="dashboard-grid">
...
export default function PaginaDeudas() {
    ...
    <Suspense fallback={
        <div className="dashboard-cuerpo">
        <div style={{ padding: '4rem 0', textAlign: 'center', color: 'var(--color-texto-suave)' }}>
            Cargando...
        </div>
        </div>
    }>
