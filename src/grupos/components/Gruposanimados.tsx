...
interface RutaAvion {
  x0: number;
  y0: number; // inicio (borde de pantalla)
  x1: number;
  y1: number; // control 1
  x2: number;
  y2: number; // control 2
  x3: number;
  y3: number; // fin (borde opuesto)
...
>[] = [
  ...
  },
  {
    ...
    burbujas: [
      {
        linea1: "Total: $485.000",
        linea2: "$121.250 p/persona",
        linea3: "LPC calculó",
        color: "#98d498",
      },
      {
        linea1: "3 deben plata",
        linea2: "notificados",
        linea3: "sin discusiones",
        color: "#c8e6c9",
      },
    ],
  },
...
export default function GruposAnimados() {
  ...
  useEffect(() => {
    ...
    function dibujarTransferencia(
      ...
      const coinX =
        (1 - tp) * (1 - tp) * ax + 2 * (1 - tp) * tp * midX + tp * tp * bx;
      const coinY =
        (1 - tp) * (1 - tp) * ay + 2 * (1 - tp) * tp * midY + tp * tp * by;
    ...
    function generarRuta(
      ...
      if (desdeDerecha) {
        x0 = W - margen;
        y0 = Math.random() * H * 0.8 + H * 0.1;
        x3 = margen;
        y3 = Math.random() * H * 0.8 + H * 0.1;
      } else {
        x0 = margen;
        y0 = Math.random() * H * 0.8 + H * 0.1;
        x3 = W - margen;
      ...
      switch (tipo) {
        case 0:
          x1raw = x0 + (x3 - x0) * 0.3;
          y1raw = Math.min(y0, y3) - H * 0.25;
          x2raw = x0 + (x3 - x0) * 0.7;
          y2raw = Math.min(y0, y3) - H * 0.15;
          break;
        case 1:
          x1raw = x0 + (x3 - x0) * 0.25;
          y1raw = y0 + H * 0.3;
        ...
        default:
          x1raw = x0 + (x3 - x0) * 0.3;
          y1raw = Math.max(y0, y3) + H * 0.2;
          x2raw = x0 + (x3 - x0) * 0.7;
          y2raw = Math.max(y0, y3) + H * 0.1;
      }
