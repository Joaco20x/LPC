...
import type { IMiembroGrupoRepository, DatosCrearMiembro, MiembroConGrupoYConteo, MiembroConUsuario } from '@/shared/repositories/IMiembroGrupoRepository';

export class PrismaMiembroGrupoRepository implements IMiembroGrupoRepository {
  async buscarPorGrupo(idGrupo: string, tx?: unknown): Promise<{ idUsuario: string }[]> {
    const client = tx as any || prisma;
    return client.miembroGrupo.findMany({ where: { idGrupo }, select: { idUsuario: true } });
  }
  ...
  async buscarMiembrosDeGrupos(idsGrupos: string[]): Promise<MiembroConUsuario[]> {
