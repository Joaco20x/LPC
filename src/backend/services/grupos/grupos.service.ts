...
import type { IDatabaseService } from '@/shared/repositories/IDatabaseService';
...
export async function crearGrupoViaje(
  ...
  return await db.transaction(async (tx) => {
    const usuariosInvitados = await usuarioRepo.buscarPorEmails(datos.correosIntegrantes);
    ...
    await miembroRepo.crearMuchas(
      [
        ...
        ...invitadosSinCreador.map((user) => ({ idGrupo: nuevoGrupo.id, idUsuario: user.id, rol: 'miembro' })),
      ],
      tx,
    );
...
export async function obtenerDetalleGrupo(idGrupo: string, grupoRepo: IGrupoRepository) {
