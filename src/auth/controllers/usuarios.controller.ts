...
export async function controladorBuscarUsuario(req: NextRequest) {
  try {
    ...
    if (!correo) {
      return NextResponse.json({ exito: false, mensaje: 'Debes proporcionar un correo' }, { status: 400 });
    }
    ...
    if (!usuario) {
      return NextResponse.json({ exito: false, mensaje: 'Usuario no encontrado' }, { status: 404 });
    }

    return NextResponse.json({
      exito: true,
      datos: { usuario: { id: usuario.id, nombre: usuario.nombre, correo: usuario.correo } },
    }, { status: 200 });
  } catch (error: any) {
    const esTokenInvalido = error.name === 'JsonWebTokenError' || error.name === 'TokenExpiredError';
    return NextResponse.json({
      exito: false,
      mensaje: esTokenInvalido ? 'Token inválido o expirado' : 'Error al buscar usuario',
