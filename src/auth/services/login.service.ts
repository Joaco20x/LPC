...
export async function procesarLogin(
  ...
  await sesionRepo.crear({ idUsuario: usuario.id, tokenHash: tokens.refreshToken, expiraEn });

  return {
    accessToken: tokens.accessToken,
    refreshToken: tokens.refreshToken,
    usuario: { id: usuario.id, nombre: usuario.nombre, correo: usuario.correo, verificado: usuario.verificado },
  };
}
