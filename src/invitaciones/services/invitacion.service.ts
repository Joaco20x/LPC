...
import type { DatosCrearInvitacion, InvitacionConEstado } from '@/invitaciones/types/invitacion';
...
export async function crearInvitacion(
  ...
  if (!esAdmin) throw new Error('Solo los administradores pueden crear invitaciones');

  // Validación por tipo
  if (datos.tipo === 'correo') {
    if (!datos.correoInvitado) throw new Error('Se requiere correo para este tipo de invitación');
    // Invalidar invitaciones previas al mismo correo en este grupo
    await invitacionRepo.invalidarPorGrupoYCorreo(datos.idGrupo, datos.correoInvitado);
  }
...
export async function aceptarInvitacion(
  ...
  if (inv.estado === 'aceptada') throw new Error('Esta invitación ya fue utilizada');
  ...
  await miembroRepo.crearMuchas([{ idGrupo: inv.idGrupo, idUsuario, rol: 'miembro' }]);
...
export async function obtenerInvitacionesGrupo(
  ...
  if (!esAdmin) throw new Error('Solo los administradores pueden ver las invitaciones');
...
export async function verificarTokenInvitacion(
  ...
  invitacionRepo: IInvitacionRepository,
  grupoRepo: IGrupoRepository,
): Promise<{ invitacion: InvitacionConEstado; nombreGrupo: string; destino: string }> {
  const inv = await invitacionRepo.buscarPorToken(token);
  if (!inv) throw new Error('Invitación no encontrada');
